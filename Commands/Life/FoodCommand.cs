...
public sealed class FoodCommand
{
    [Command("food")]
    [CommandSyntax("[player]")]
    [CommandDescription("Allows to set max value of your or somebody's food.")]
    [RegisterCommandPermission("other", Description = "Allows to set max value of other player's food.")]
        ...
        protected override async UniTask OnExecuteAsync()
        {
            ...
            if (!m_AdminSystem.IsInAdminMode(user))
                throw new UserFriendlyException(string.Format("{0}{1}",
                     m_StringLocalizer["food_command:prefix"],
        ...
        IServiceProvider serviceProvider) : UnturnedCommand(serviceProvider)
    {
        ...
        protected override async UniTask OnExecuteAsync()
        {
            ...
            await PrintAsync(m_StringLocalizer["food_command:somebody:executor", new
            {
                PlayerName = sPlayer.playerID.playerName,
                CharacterName = sPlayer.playerID.characterName,
                NickName = sPlayer.playerID.nickName,
