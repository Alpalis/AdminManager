...
namespace Alpalis.AdminManager.Commands.Life
{
    public class FoodCommand
    {
        ...
        [CommandDescription("Command to update food for yourself and other players.")]
        [RegisterCommandPermission("other", Description = "Allows to set food of other player.")]
        [CommandActor(typeof(UnturnedUser))]
        #endregion Command Parameters
        public class FoodUnturned : UnturnedCommand
        {
            ...
            protected override async UniTask OnExecuteAsync()
            {
                ...
                if (!m_AdminSystem.IsInAdminMode(user))
                    throw new UserFriendlyException(string.Format("{0}{1}",
                         config.MessagePrefix ? m_StringLocalizer["food_command:prefix"] : "",
                         m_StringLocalizer["food_command:error_adminmode"]));
                if (Context.Parameters.Count == 0)
                {
                    ...
                    PrintAsync(string.Format("{0}{1}",
                        config.MessagePrefix ? m_StringLocalizer["food_command:prefix"] : "",
                        m_StringLocalizer["food_command:yourself"]));
                    return;
                }
                ...
                if (!Context.Parameters.TryGet(0, out UnturnedUser? targetUser) || targetUser == null)
                    throw new UserFriendlyException(string.Format("{0}{1}",
                        config.MessagePrefix ? m_StringLocalizer["food_command:prefix"] : "",
                        m_StringLocalizer["food_command:error_player"]));
                ...
                targetUser.PrintMessageAsync(string.Format("{0}{1}",
                    config.MessagePrefix ? m_StringLocalizer["food_command:prefix"] : "",
                    m_StringLocalizer["food_command:somebody:player", new
                    {
                ...
                PrintAsync(string.Format("{0}{1}",
                    config.MessagePrefix ? m_StringLocalizer["food_command:prefix"] : "",
                    m_StringLocalizer["food_command:somebody:executor", new
                    {
        ...
        [Command("food")]
        [CommandSyntax("<player>")]
        [CommandDescription("Command to set food of other players.")]
        [CommandActor(typeof(ConsoleActor))]
        #endregion Command Parameters
        public class FoodConsole : UnturnedCommand
        {
            #region Member Variables
            private readonly IIdentityManagerImplementation m_IdentityManagerImplementation;
            private readonly IStringLocalizer m_StringLocalizer;
            ...
            protected override async UniTask OnExecuteAsync()
            {
                ...
                if (!Context.Parameters.TryGet(0, out UnturnedUser? user) || user == null)
                    throw new UserFriendlyException(m_StringLocalizer["food_command:error_player"]);
                SteamPlayer sPlayer = user.Player.SteamPlayer;
                CSteamID steamID = sPlayer.playerID.steamID;
                ushort? identity = m_IdentityManagerImplementation.GetIdentity(steamID);
                await UniTask.SwitchToMainThread();
                ...
                user.PrintMessageAsync(string.Format("{0}{1}",
                    config.MessagePrefix ? m_StringLocalizer["food_command:prefix"] : "",
                    m_StringLocalizer["food_command:somebody:console"]));
                PrintAsync(m_StringLocalizer["food_command:somebody:executor", new
                {
                    PlayerName = sPlayer.playerID.playerName,
                    CharacterName = sPlayer.playerID.characterName,
                    NickName = sPlayer.playerID.nickName,
